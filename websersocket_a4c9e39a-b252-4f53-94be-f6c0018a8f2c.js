    ...
    if (s_path === '/api/file') {
        ...
        try {
            ...
            if(s_range){
                ...
                let n_total = o_stat.size;
                let a_s_part = s_range.replace(/bytes=/, '').split('-');
                let n_start = parseInt(a_s_part[0], 10);
                let n_end = a_s_part[1] ? parseInt(a_s_part[1], 10) : n_total - 1;
                let n_chunk = n_end - n_start + 1;
                let o_file = await Deno.open(s_path_file, { read: true });
                await o_file.seek(n_start, Deno.SeekMode.Start);
                let a_n_buf = new Uint8Array(n_chunk);
                await o_file.read(a_n_buf);
                o_file.close();
                return new Response(a_n_buf, {
                    status: 206,
                    headers: {
                        'content-type': s_content_type,
                        'content-range': `bytes ${n_start}-${n_end}/${n_total}`,
                        'accept-ranges': 'bytes',
                        'content-length': String(n_chunk),
                    },
                });
            }
            return new Response(a_n_byte, {
                headers: { 'content-type': s_content_type, 'accept-ranges': 'bytes' },
            });
        } catch {
